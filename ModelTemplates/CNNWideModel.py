...
def createModel(random_seed_value, input_x, input_y, color_depth):
    ...
    #Convolution 1
    model.add(tf.keras.layers.Conv2D(filters = 64, kernel_size = 5, padding = 'same', strides = 1, input_shape = (input_x, input_y, color_depth), name = 'Convolution_1'))
    #ReLU 2
    ...
    #Convolution 4
    model.add(tf.keras.layers.Conv2D(filters = 128, kernel_size = 5, padding = 'same', strides = 1, name = 'Convolution_4'))
    #ReLU 5
