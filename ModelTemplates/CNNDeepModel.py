...
def createModel(random_seed_value, input_x, input_y, color_depth):
    ...
    #Convolution 7
    model.add(tf.keras.layers.Conv2D(filters = 128, kernel_size = 5, padding = 'same', strides = 1, name = 'Convolution_7'))
    #ReLU 8
    model.add(tf.keras.layers.Activation(tf.nn.relu, name = 'ReLU_8'))
    #Max pooling 9
    model.add(tf.keras.layers.MaxPooling2D(pool_size = 2, strides = 2, name = 'Max_pooling_9'))
    #Flatten 10
